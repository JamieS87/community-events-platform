...
test("Users can log in, and are redicted to the homepage on success", async ({
  page,
}) => {
  await page.goto("/");
...
test("Users are show a message when providing unrecognized credentials", async ({
  page,
}) => {
  await page.goto("/");
  await page.getByTestId("login").click();
  ...
  await page.getByTestId("signin").click();
  await expect(page.getByTestId("signin-alert")).toHaveText(
